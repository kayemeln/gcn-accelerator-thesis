// integrated_gcn_photo_tb.cpp
//
// Testbench for the reordered GCN layer on Amazon Photo: H_out = A * (X * W)
//
// Input files (from photo_scripts/photo.py):
...
int main(int argc, char* argv[]) {
    ...
    //    Uncomment ONE layer (must match header config)
    // ----------------------------------------------------------
    ...
    int errors = compare("GCN Layer (Photo): A*(X*W)", H_out_f, H1_ref, NODES, F_OUT);
