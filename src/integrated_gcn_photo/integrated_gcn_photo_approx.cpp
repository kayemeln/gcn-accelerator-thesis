// Approximate-multiply variant of the GCN accelerator (SpMM stage only).
// Computation order: H_out = A * (X * W)
// Stage 1: GEMM  — H_temp = X * W   (exact multiplication)
// Stage 2: SpMM  — H_out  = A * H_temp (approximate multiplication)
//
// SpMM MAC uses gate-level approximate multipliers from EvoApproxLib,
// selected via APPROX_MUL_VARIANT. GEMM keeps exact multiplication to
// preserve accuracy on the dense feature reduction.
// Configured for Amazon Photo dataset.

#include "integrated_gcn_photo.h"
#include "approx_mul/approx_mul.h"

// ================================================================
//  STAGE 1: GEMM  (tiled block matrix multiply, exact multiply)
//  H_temp = X * W
// ================================================================
...
// ================================================================
//  STAGE 2: SpMM  (sparse matrix * dense, approximate multiply)
// ================================================================
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=1435 avg=32
...
#pragma HLS PIPELINE II=1
#pragma HLS UNROLL factor=UF
                acc[f] += (acc32_t)approx_mul(val, h_buf[f]);
            }
        }
...
// ================================================================
//  TOP-LEVEL: GCN Layer (approximate SpMM)  —  A * (X * W)
// ================================================================
...
#pragma HLS INTERFACE mode=m_axi bundle=gmem6 port=H_out
    ...
    // Stage 1: GEMM  (X * W → H_temp)  — exact multiply
    gemm_stage(X, W, H_temp);

    // Stage 2: SpMM  (A * H_temp → H_out)  — approximate multiply
