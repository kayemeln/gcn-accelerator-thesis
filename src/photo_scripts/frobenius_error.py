...
references produced by photo.py.

    relative error = ||HLS - ref||_F / ||ref||_F

Comparisons:
    Layer 1: H_out_design3.bin  vs  H1.bin   (7650 x 64)
    Layer 2: H_out_layer2.bin   vs  H2.bin   (7650 x 8)
"""
...
REF_DIR = os.path.join(SCRIPT_DIR, "photo_bin_sparse")
HLS_DIR = REF_DIR

COMPARISONS = [
    {
        "label": "Layer 1",
        "hls_file": os.path.join(HLS_DIR, "H_out_integrated.bin"),
        "ref_file": os.path.join(REF_DIR, "H1.bin"),
        "shape": (NODES, 64),
    },
    {
        "label": "Layer 2",
        "hls_file": os.path.join(HLS_DIR, "H_out_integrated_layer2.bin"),
