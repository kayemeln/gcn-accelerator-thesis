// design3_gcn_tb.cpp
//
// Testbench for the reordered GCN layer: H_out = A * (X * W)
//
// Input files:
...
int main(int argc, char* argv[]) {
    ...
    // Layout: X_vec[(f * NODES_PAD + i) / DSIZE][(f * NODES_PAD + i) % DSIZE]
    // Padding rows [F_IN..F_IN_PAD_GEMM-1] and padding cols [NODES..NODES_PAD-1]
    ...
    // 4. Run DUT
    // ----------------------------------------------------------
    std::cout << "\nRunning design3_gcn (A*(X*W) order)..." << std::endl;
