...
typedef struct {
    ...
    XNaive_spmm_stage_uio_map maps[ MAX_UIO_MAPS ];
} XNaive_spmm_stage_uio_info;

/***************** Variable Definitions **************************************/
static XNaive_spmm_stage_uio_info uio_info;
...
static int uio_info_read_name(XNaive_spmm_stage_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XNaive_spmm_stage_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XNaive_spmm_stage_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XNaive_spmm_stage_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XNaive_spmm_stage_Initialize(XNaive_spmm_stage *InstancePtr, const char* InstanceName) {
	XNaive_spmm_stage_uio_info *InfoPtr = &uio_info;
