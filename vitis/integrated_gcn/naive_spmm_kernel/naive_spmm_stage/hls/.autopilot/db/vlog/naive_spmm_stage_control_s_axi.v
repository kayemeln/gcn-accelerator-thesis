...
// ==============================================================
`timescale 1ns/1ps
(* DowngradeIPIdentifiedWarnings="yes" *) module naive_spmm_stage_control_s_axi
#(parameter
...
//------------------------Parameter----------------------
localparam
    ADDR_ROWPTR_DATA_0 = 7'h10,
    ADDR_ROWPTR_DATA_1 = 7'h14,
    ADDR_ROWPTR_CTRL   = 7'h18,
...
always @(posedge ACLK) begin
    if (ACLK_EN) begin
        if (ar_hs) begin
            rdata <= 'b0;
            case (raddr)
                ADDR_ROWPTR_DATA_0: begin
                    rdata <= int_rowptr[31:0];
                end
                ADDR_ROWPTR_DATA_1: begin
                    rdata <= int_rowptr[63:32];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_ROWPTR_DATA_0)
            int_rowptr[31:0] <= (WDATA[31:0] & wmask) | (int_rowptr[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_ROWPTR_DATA_1)
            int_rowptr[63:32] <= (WDATA[31:0] & wmask) | (int_rowptr[63:32] & ~wmask);
    end
end
