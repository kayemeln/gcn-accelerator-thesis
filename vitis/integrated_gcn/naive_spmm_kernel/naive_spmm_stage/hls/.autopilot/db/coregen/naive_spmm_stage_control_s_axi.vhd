...
entity naive_spmm_stage_control_s_axi is
generic (
...
end entity naive_spmm_stage_control_s_axi;
...
architecture behave of naive_spmm_stage_control_s_axi is
...
attribute DowngradeIPIdentifiedWarnings of behave : architecture is "yes";
    ...
    constant ADDR_ROWPTR_DATA_0 : INTEGER := 16#10#;
    constant ADDR_ROWPTR_DATA_1 : INTEGER := 16#14#;
    constant ADDR_ROWPTR_CTRL   : INTEGER := 16#18#;
    constant ADDR_COLIND_DATA_0 : INTEGER := 16#1c#;
    constant ADDR_COLIND_DATA_1 : INTEGER := 16#20#;
    constant ADDR_COLIND_CTRL   : INTEGER := 16#24#;
    constant ADDR_VALUES_DATA_0 : INTEGER := 16#28#;
    constant ADDR_VALUES_DATA_1 : INTEGER := 16#2c#;
    constant ADDR_VALUES_CTRL   : INTEGER := 16#30#;
    constant ADDR_H_TEMP_DATA_0 : INTEGER := 16#34#;
    constant ADDR_H_TEMP_DATA_1 : INTEGER := 16#38#;
    constant ADDR_H_TEMP_CTRL   : INTEGER := 16#3c#;
    constant ADDR_H_OUT_DATA_0  : INTEGER := 16#40#;
    constant ADDR_H_OUT_DATA_1  : INTEGER := 16#44#;
    constant ADDR_H_OUT_CTRL    : INTEGER := 16#48#;
    ...
    -- internal registers
    signal int_rowptr          : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_ROWPTR_DATA_0 =>
                        rdata_data <= RESIZE(int_rowptr(31 downto 0), 32);
                    when ADDR_ROWPTR_DATA_1 =>
                        rdata_data <= RESIZE(int_rowptr(63 downto 32), 32);
...
-- ----------------------- Register logic ----------------
    rowptr               <= STD_LOGIC_VECTOR(int_rowptr);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_rowptr(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_ROWPTR_DATA_0) then
                    int_rowptr(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_rowptr(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_rowptr(63 downto 32) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_ROWPTR_DATA_1) then
                    int_rowptr(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_rowptr(63 downto 32));
                end if;
            end if;
        end if;
    end process;
